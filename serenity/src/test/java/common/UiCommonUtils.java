...
//    if (SessionVariableUi.LANGUAGE_VERSION.get().equals(Language.DEUTSCH)) {
//      amount = amount.substring(0, amount.length() - 2);
//    } else if (SessionVariableUi.LANGUAGE_VERSION.get().equals(Language.ENGLISH)) {
...
//    String language = SessionVariableUi.LANGUAGE_VERSION.get().toString();
...
//    return voucherTypeResponse.getVoucherAttributes().stream().anyMatch(a -> a.getName().equals(attributeName)
//      && voucherTypeResponse.getVoucherAttributes().stream().filter(b -> b.getName().equals(attributeName)).findAny()
//      .get().getValue() != null);
//  }
//
//  public static Boolean voucherTypeAttributeIsMandatoryAndEmpty(VoucherTypeResponse voucherTypeResponse,
//                                                                String attributeName) {
//    return voucherTypeResponse.getVoucherAttributes().stream().anyMatch(a -> a.getName().equals(attributeName)
//      && voucherTypeResponse.getVoucherAttributes().stream().filter(b -> b.getName().equals(attributeName)).findAny()
//      .get().getMandatory() && !voucherTypeAttributeIsPresent(voucherTypeResponse, attributeName));
//  }
//
//  public static String getVoucherTypeAttribute(VoucherTypeResponse voucherTypeResponse,
//                                               String attributeName) {
//    return voucherTypeResponse.getVoucherAttributes().stream().filter(e -> e.getName().equals(attributeName)).findFirst().get()
//      .getValue();
//  }
  ...
  public static void clickOnTargetWithJavascript(Actor actor, Target target) {
    ...
    int x = location.getX();
    int y = location.getY();
    WebDriver driver = SessionVariableUi.BROWSER.get();
  ...
  public static String getLocaleFromLocalStorage(WebDriver driver) {
    return (String) ((JavascriptExecutor) driver).executeScript("return window.localStorage.getItem('user.locale');");
  }
  ...
  public static void waitForLoginPage() {
    ...
    wait.until(ExpectedConditions.not(ExpectedConditions.textToBe(By.className("qa_login_page_title"), "")));
  }

  public static void waitPageIsCompleted(Actor actor, Target target) {
    WebDriver driver = SessionVariableUi.BROWSER.get();
    WebDriverWait webDriverWait = new WebDriverWait(driver, 15);
    webDriverWait.until((Function<WebDriver, Boolean>) input -> ((JavascriptExecutor) driver)
...
//    wait.until(ExpectedConditions.not(ExpectedConditions.textToBePresentInElementValue(element, "")));
    wait.until(ExpectedConditions.textMatches(By.className("qa_top_bar_title"), Pattern.compile("\\s*")));
  }
  ...
  public static void waitForData() {
    ...
    try {
      wait.until(ExpectedConditions.and(
        ExpectedConditions.invisibilityOfElementLocated(By.className("ui-datatable-emptymessage")),
...
//        ExpectedConditions.not(ExpectedConditions.attributeContains(By.className("block-ui"), "class", "enabled"))
  ...
  public static String formatVoucherTypeLastChangeDate(String displayedValueAsPattern, ZonedDateTime lastChange) {
    if (SessionVariableUi.LANGUAGE_VERSION.get().toString().equals("English")) {
  ...
  public static String formatDateToDisplayedValue(Target displayedValueAsPattern, ZonedDateTime date) {
    Actor actor = SessionVariableUi.USER.get();
    String displayedText = Text.of(displayedValueAsPattern).viewedBy(actor).asString();
    String pattern = getPattern(displayedText);

    switch (SessionVariableUi.LANGUAGE_VERSION.get().toString()) {
      case "English":
        return date.withZoneSameInstant(ZoneId.systemDefault()).format(DateTimeFormatter.ofPattern(pattern)
...
//    dateAsPattern = removeUnnecessaryZerosFromDate(dateAsPattern);
    ...
    for (String parse : FORMAT_STRINGS) {
      try {
        ZonedDateTime.parse(dateAsPattern, DateTimeFormatter.ofPattern(parse, locale).withZone(ZoneId.systemDefault()));
        return parse;
      } catch (DateTimeParseException e) {
        try {
          System.out.println("Failed with ZonedDateTime.parse by parser " + parse);
          LocalDate.parse(dateAsPattern, DateTimeFormatter.ofPattern(parse, locale).withZone(ZoneId.systemDefault()));
          return parse;
    ...
    throw new DateTimeParseException("This string can't be parsed: ".concat(dateAsPattern), dateAsPattern, 0);
  }
  ...
  public static ZonedDateTime createLocalDateTimeFromDisplayedValue(String date) {
    ...
    return ZonedDateTime.parse(date, DateTimeFormatter.ofPattern(getPattern(date)).withZone(ZoneId.systemDefault()));
...
//    switch (month) {
//      case JANUARY:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.JANUARY.translate(), year);
//      case FEBRUARY:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.FEBRUARY.translate(), year);
//      case MARCH:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.MARCH.translate(), year);
//      case APRIL:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.APRIL.translate(), year);
//      case MAY:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.MAY.translate(), year);
//      case JUNE:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.JUNE.translate(), year);
//      case JULY:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.JULY.translate(), year);
//      case AUGUST:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.AUGUST.translate(), year);
//      case SEPTEMBER:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.SEPTEMBER.translate(), year);
//      case OCTOBER:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.OCTOBER.translate(), year);
//      case NOVEMBER:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.NOVEMBER.translate(), year);
//      case DECEMBER:
//        return prepareValueForCalendar(day, MonthsShortFormatTranslations.DECEMBER.translate(), year);
//      default:
...
//    }
  ...
  private static String prepareValueForCalendar(int day, String month, int year) {
    switch (SessionVariableUi.LANGUAGE_VERSION.get().toString()) {
      case "English":
        return month.concat(" ").concat(String.valueOf(day)).concat(", ").concat(String.valueOf(year));
      case "Deutsch":
        return String.valueOf(day).concat(". ").concat(month).concat(" ").concat(String.valueOf(year));
      default:
        return "Not supported language";
    }
  }
  ...
  public static String getDefaultUrl() {
    ...
    return UiConfiguration.get().getUrl();
  }
  ...
  public static Integer getExampleNumberFromDescription(Integer testRun) {
    String description = "";
    try {
      List<TestStep> testSteps = StepEventBus.getEventBus().getBaseStepListener().getTestOutcomes().get(testRun).getTestSteps();
